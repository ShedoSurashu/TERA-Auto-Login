...
#AutoIt3Wrapper_Outfile=bin\teralogin.exe
...
$prompt = IniRead("teralogin.ini", "account", "prompt", "")
...
; If the prompt value is not 'yes' or 'no' then it defaults to 'yes'.
If ($prompt <> 'yes' And $prompt <> 'no') Then
	$prompt = 'yes'
EndIf
...
; This checks if the prompt is enabled in the settings. If not, it will skip the prompting.
If ($prompt = 'yes') Then
